...
class Callback implements HttpPostActionInterface, CsrfAwareActionInterface
{
    ...
    private function handleLocked(
        ...
        $order = $this->bogOrderResolver->findOrder($externalOrderId, $bogOrderId);
        ...
        if ($order === null) {
            ...
            $quoteId = $this->bogOrderResolver->findQuoteIdByBogOrderId($bogOrderId);
            if ($quoteId === null) {
                $this->logger->info(
            ...
            $order = $this->bogOrderResolver->materializeOrderFromQuote($quoteId, $bogOrderId);
            if ($order === null) {
                return 'ERROR';
            }
        }
